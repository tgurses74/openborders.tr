...
// .wrangler/tmp/pages-gbYs4E/functionsWorker-0.9532925597400994.mjs
var __defProp2 = Object.defineProperty;
var __name2 = /* @__PURE__ */ __name((target, value) => __defProp2(target, "name", { value, configurable: true }), "__name");
var onRequestGet = /* @__PURE__ */ __name2(async ({ request, env, waitUntil }) => {
  const token = new URL(request.url).searchParams.get("token") || "";
  const fail = /* @__PURE__ */ __name2((reason) => Response.redirect(`${env.SITE_URL}/kayit-onay/?err=${reason}`, 302), "fail");
  ...
  if (user.status !== "confirmed" && new Date(user.token_expires_at) < /* @__PURE__ */ new Date()) {
    return fail("expired");
  }
...
async function syncHubspot(env, user) {
  try {
    ...
    if (res.ok || res.status === 409) {
      const id = res.ok ? (await res.json()).id : null;
      await env.DB.prepare("UPDATE users SET hubspot_synced=1, hubspot_contact_id=? WHERE id=?").bind(id, user.id).run();
    } else {
...
__name(syncHubspot, "syncHubspot");
__name2(syncHubspot, "syncHubspot");
...
var onRequestPost = /* @__PURE__ */ __name2(async ({ request, env }) => {
  ...
  const expires = new Date(Date.now() + 48 * 3600 * 1e3).toISOString();
  await env.DB.prepare(
  ...
  ).bind(email, firstName, lastName, phone, interest, lang, token, expires).run();
  const confirmUrl = `${env.SITE_URL}/api/confirm?token=${token}`;
  const subject = lang === "tr" ? "Openborders kayd\u0131n\u0131z\u0131 onaylay\u0131n" : "Confirm your Openborders registration";
  const html = lang === "tr" ? `<p>Merhaba ${escapeHtml(firstName)},</p>
         <p>Openborders kayd\u0131n\u0131z\u0131 tamamlamak i\xE7in l\xFCtfen a\u015Fa\u011F\u0131daki ba\u011Flant\u0131ya t\u0131klay\u0131n:</p>
         <p><a href="${confirmUrl}">Kayd\u0131m\u0131 Onayla</a></p>
         <p>Bu kayd\u0131 siz ba\u015Flatmad\u0131ysan\u0131z bu e-postay\u0131 yok sayabilirsiniz. Ba\u011Flant\u0131 48 saat ge\xE7erlidir.</p>` : `<p>Hello ${escapeHtml(firstName)},</p>
...
// .wrangler/tmp/bundle-qqct5N/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-qqct5N/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9532925597400994.js.map
